...
export async function addToCart(
  cartId: string,
  lineItem: Array<{ merchandiseId: string; quantity: number }> // changed here logic to work with shopify and medusa
): Promise<Cart> {
  const res = await medusaRequest({
    method: 'POST',
    path: `/carts/${cartId}/line-items`,
    payload: {
      variant_id: lineItem[0]?.merchandiseId, // changed here varientId just name to work with shopify and medusa
...
export async function updateCart(
  ...
  const quantityToUpdate = lineItems[0]?.quantity;
  const res = await medusaRequest({
    method: 'POST',
    path: `/carts/${cartId}/line-items/${lineItems[0]?.id}`,
    payload: {
...
export async function getCategoryProducts(
  ...
  // reverse?: boolean,
  // sortKey?: string
  { collection, reverse, sortKey }: { collection: string; reverse?: boolean; sortKey?: string }
): Promise<Product[]> {
  const res = await medusaRequest({
    method: 'GET',
    path: `/product-categories?handle=${collection}`,
    tags: ['categories']
  });
